...
def go(args: argparse.Namespace) -> None:
    ...
    logger.info(f"* Loading metadata source: {args.metadata}")

    md_reader = GGUFReader(args.metadata, "c")

    if args.tensors is not None:
        logger.info(f"* Loading tensor data source: {args.metadata}")
        td_reader: None | GGUFReader = GGUFReader(args.tensors, "c")
    else:
        td_reader = None
        logger.info("* No tensor data source specified, output will be a vocab-only model.")
    arch = str(
    ...
    logger.info("* Adding metadata: ", end="")
    ...
    if td_reader is not None:
        logger.info("* Adding tensor metadata: ", end="")
        for tensor in td_reader.tensors:
            logger.info(f"{tensor.name!r}, ", end="")
            gw.add_tensor_info(
                tensor.name,
...
def main() -> None:
    ...
    if args.output in (args.metadata, args.tensors):
        logger.error("! Cannot overwrite input files with --out", file=sys.stderr)
        sys.exit(1)

    if not args.force and Path(args.output).exists():
        logger.error("! Output file already exists", file=sys.stderr)
        sys.exit(1)
    go(args)
